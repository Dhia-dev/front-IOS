...
class LikeViewModel {
    ...
    func getMyLikes(completed: @escaping (Bool, [Like]?) -> Void ) {
        AF.request(Constants.serverUrl + "/like/my",
                   method: .post,
                   parameters: [
    ...
    func getLikeById(_id: String?, completed: @escaping (Bool, Like?) -> Void ) {
        AF.request(Constants.serverUrl + "/like/by-id",
                   method: .get,
                   parameters: [
                    "_id": _id!
                   ])
    ...
    func addLike(like: Like, completed: @escaping (Bool) -> Void ) {
        AF.request(Constants.serverUrl + "/like",
                   method: .post,
                   parameters: [
                    "seen": like.seen!,
    ...
    func deleteLike(_id: String?, completed: @escaping (Bool) -> Void ) {
        AF.request(Constants.serverUrl + "/like/del",
                   method: .post,
                   parameters: [
                    "_id": _id!
                   ])
    ...
    func makeLike(jsonItem: JSON) -> Like {
        Like(
            _id: jsonItem["_id"].stringValue,
             clickDate: Date(), //jsonItem["clickDate"].stringValue,
            seen: jsonItem["seen"].boolValue,
            liker: makeUser(jsonItem: jsonItem["liker"]),
            liked: makeUser(jsonItem: jsonItem["liked"])
        )
    }
